...
def _summary(split_scores):
    ...
    for target in ("Valence", "Arousal"):
        for model, score in split_scores.items():
            experiments.append({"target": target, "model": model, "status": "ok",
                                "macro_f1": score, "roc_auc": score})
    return {"experiments": experiments}


def test_robust_ranking_excludes_repo_split(tmp_path):
    summaries = {
        "subject": _summary({"stable": .70, "repo_star": .65}),
        "trial": _summary({"stable": .68, "repo_star": .64}),
        "repo": _summary({"stable": .60, "repo_star": .99}),
    }
    result = write_cross_split_reports(tmp_path, summaries)
    assert result["best_robust_model"] == {
        "Valence": "stable", "Arousal": "stable"
    }
...
def test_model_must_complete_both_generalization_splits(tmp_path):
    summaries = {
        "subject": _summary({"complete": .60, "subject_only": .95}),
        "trial": _summary({"complete": .61}),
    }
    result = write_cross_split_reports(tmp_path, summaries)
    incomplete = next(row for row in result["models"]
