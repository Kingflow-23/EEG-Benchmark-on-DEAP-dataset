...
def write_reports(
    ...
    for target in ("Valence", "Arousal"):
        ...
        ranked = sorted(
            candidates,
            key=lambda r: tuple(r.get(k, float("-inf")) for k in RANK_KEYS),
            reverse=True,
        )
    ...
    columns = sorted(
        {key for row in records for key in row if key != "confusion_matrix"}
    )
    with (output / "comparison.csv").open("w", newline="", encoding="utf-8") as handle:
        writer = csv.DictWriter(handle, fieldnames=columns)
        writer.writeheader()
        writer.writerows(
            {k: v for k, v in row.items() if k != "confusion_matrix"} for row in records
        )
    lines = [
        "# DEAP architecture benchmark",
        "",
        f"Primary ranking: `{RANK_KEYS[0]}`; ties: `{RANK_KEYS[1]}`, then `{RANK_KEYS[2]}`.",
        "",
    ...
    for row in sorted(successful, key=lambda r: (r["target"], r.get("rank", 999))):
        lines.append(
            f"| {row['target']} | {row.get('rank','')} | {row['model']} | {row['macro_f1']:.4f} | {row['roc_auc']:.4f} | {row['accuracy']:.4f} | {row['accuracy_lift']:.4f} | {row['train_seconds']:.2f} | {row['inference_seconds']:.2f} |"
        )
    failed = [r for r in records if r.get("status") != "ok"]
    if failed:
        lines += ["", "## Skipped or failed", ""] + [
            f"- `{r['target']}/{r['model']}`: {r.get('error', r['status'])}"
            for r in failed
        ]
...
def _write_comparison_plot(output: Path, records: list[dict[str, Any]]) -> None:
    ...
    fig, axes = plt.subplots(1, 2, figsize=(12, max(4, len(records) * 0.22)))
    for ax, target in zip(axes, targets):
        rows = sorted(
            (row for row in records if row["target"] == target),
...
def write_cross_split_reports(
    ...
    for target in ("Valence", "Arousal"):
        ...
        for model, split_records in by_model.items():
            ...
            if eligible:
                scores = [
                    split_records[split]["macro_f1"] for split in generalization_splits
                ]
    ...
    for target in ("Valence", "Arousal"):
        ranked = sorted(
            ...
            key=lambda row: (
                row["generalization_mean_macro_f1"],
    ...
    winners = {
        target: next(
            (
                row["model"]
                for row in rows
                if row["target"] == target and row.get("robustness_rank") == 1
            ),
            None,
        )
        for target in ("Valence", "Arousal")
    ...
    for row in sorted(
        (item for item in rows if item.get("robustness_rank")),
