...
def evaluate_probabilities(y_true: np.ndarray, probabilities: np.ndarray) -> dict[str, Any]:
    ...
    if (not np.all(np.isfinite(proba)) or np.any(proba < 0) or np.any(proba > 1)
    ...
    return {
        ...
        "f1_low": float(f1_score(y_true, pred, labels=[0], average=None, zero_division=0)[0]),
        "f1_high": float(f1_score(y_true, pred, labels=[1], average=None, zero_division=0)[0]),
...
def save_evaluation_artifacts(directory: Path, y_true: np.ndarray, probabilities: np.ndarray,
                              metrics: dict[str, Any],
                              metadata: np.ndarray | None = None) -> None:
    ...
    payload = {"y_true": y_true, "probabilities": probabilities,
    ...
    try:
        ...
        ConfusionMatrixDisplay(np.asarray(metrics["confusion_matrix"]), display_labels=["Low", "High"]).plot(ax=ax)
        fig.tight_layout(); fig.savefig(directory / "confusion_matrix.png", dpi=160); plt.close(fig)
        ...
        for name, x, y, xlabel, ylabel in (
            ...
            ("calibration_curve", mean, frac, "Mean predicted probability", "Observed frequency"),
        ):
            fig, ax = plt.subplots(figsize=(5, 4)); ax.plot(x, y, marker=".")
            if name != "precision_recall_curve": ax.plot([0, 1], [0, 1], "--", color="grey")
            ax.set(xlabel=xlabel, ylabel=ylabel, title=name.replace("_", " ").title())
            fig.tight_layout(); fig.savefig(directory / f"{name}.png", dpi=160); plt.close(fig)
    except ImportError:
        (directory / "PLOTS_SKIPPED.txt").write_text("Install matplotlib to generate plots.\n", encoding="utf-8")
